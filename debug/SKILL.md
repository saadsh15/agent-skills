...
4. **The Environment:** What OS, language version, or framework version is this running on? Are there relevant environment variables? You can run [check_environment.py](file:///home/saad/.gemini/config/skills/debug/scripts/check_environment.py) to automatically extract system diagnostics, runtime versions, listening port statuses, and local environment variables.
...
1. **Trace the Stack & Logs:** Use your file search and read tools to follow the stack trace down to the exact line of failure. If you have logs, you can run/pipe them through the [log_analyzer.py](file:///home/saad/.gemini/config/skills/debug/scripts/log_analyzer.py) script to trace exception details.
...
4. **Test Endpoints:** If debugging API or web service failures, utilize the [test_api.py](file:///home/saad/.gemini/config/skills/debug/scripts/test_api.py) helper script to run request checks with custom methods, payloads, and headers.
5. **Isolate the Component:** Can the bug be reproduced in a standalone script or unit test? If so, write the test and ask the user to run it.
